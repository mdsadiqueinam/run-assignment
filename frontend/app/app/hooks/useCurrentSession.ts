...
// Helper function for standalone exports
function sendTabMessageStandalone(message: string) {
  if (typeof window !== "undefined") {
    const messageData = JSON.stringify({ origin: TAB_ID, message });
    window.localStorage.setItem("tabMessage", messageData);
  }
}
...
export const logoutCurrentSession = async () => {
  ...
  sessionStorage.removeItem("isLogin");
  sendTabMessageStandalone("logout");
  window.location.href = "/signin";
};

export async function hydrateSession() {
  const response = await fetch("/auth/hydrateSession.json");
...
export function havePermissionsChanged(oldCurrentSessionVal: any) {
  return (
    oldCurrentSessionVal?.userPermissionId !== currentSession?.userPermissionId
  );
}
